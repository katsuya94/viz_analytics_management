class SessionsPercentDirect < Calculated
	...
	def self.calculate(sources)
		100.0 * sources[:sessions_direct] / (sources[:sessions_direct] + sources[:sessions_search] + sources[:sessions_referral] + sources[:sessions_social] + sources[:sessions_email])
	end
